Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `openserbia/watchtower` image. If you are using ARM based architecture, pull the appropriate `openserbia/watchtower:armhf-<tag>` image from the [openserbia Docker Hub](https://hub.docker.com/r/openserbia/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  openserbia/watchtower container_to_watch --debug
```
