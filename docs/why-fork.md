...
| Shoutrrr library                 | `containrrr/shoutrrr` (paused)     | **`nicholas-fedor/shoutrrr`** v0.16 (active fork, URL-compatible)                           |
| Docker Engine SDK                | `github.com/docker/docker` (deprecated, `+incompatible`) | **`github.com/moby/moby/{api,client}`** — the maintained, semver'd Docker v29 SDK (post-v29 engine and CVE fixes land only here) |
...
| Local builds on containerd image store                 | Noisy `pull access denied for app` on every poll (heuristic ignores containerd-snapshotter RepoDigest synthesis) | **Reads the daemon's per-image `Identity` provenance record** on API v1.53+ (the v29 SDK client defaults to API 1.54 and negotiates down to the daemon) with a bare-name pull-error safeguard for older daemons                                                                                         |
